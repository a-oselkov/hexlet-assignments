...
public final class UserController {
    ...
    public static Handler createUser = ctx -> {
        ...
        Validator<String> emailValidator = ctx.formParamAsClass("email", String.class)
                .check(it -> EmailValidator.getInstance().isValid(it), "не верный формат email");

        Validator<String> passwordValidator = ctx.formParamAsClass("password", String.class)
                .check(StringUtils::isNumeric, "должен содержать только цифры")
                .check(it -> it.length() > 3, "должен быть не короче 4 символов");
        ...
        if (!errors.isEmpty()) {
            ...
            ctx.status(422);
            // Передаем ошибки и данные пользователя
