...
public class WebSecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(HttpSecurity http) throws Exception {
        ...
        http
                .authorizeRequests()
                // Просмотр главной страницы и Регистрация доступна всем пользователям
                .antMatchers(GET, "/").permitAll()
                .antMatchers(POST, "/users").permitAll()
                // Просматривать список пользователей и информацию о конкретном
                // с полномочиями "USER" или "ADMIN"
                .antMatchers(GET,"/users").hasAnyAuthority(UserRole.USER.name(),
                        UserRole.ADMIN.name())
